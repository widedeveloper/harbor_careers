<template>
    <div class="app" v-if="loaded">
        ...
        <!--Section 1-->
        <div v-for="(slide, index) in slideImgs" :key="index">
            <!-- <div class="section section-1 img-6" :style="{'background-image': `url(${require('../assets/images/dypitch-5.jpg')})`}"></div>
            <div class="section section-1 img-5" :style="{'background-image': `url(${require('../assets/images/dypitch-4.jpg')})`}"></div>
            <div class="section section-1 img-4" :style="{'background-image': `url(${require('../assets/images/dypitch-3.jpg')})`}"></div>
            <div class="section section-1 img-3" :style="{'background-image': `url(${require('../assets/images/dypitch-2.jpg')})`}"></div>
            <div class="section section-1 img-2" :style="{'background-image': `url(${require('../assets/images/dypitch-1.jpg')})`}"></div> -->

            <div :class="`section section-1 img-${index + 1}`" :style="{'background-image': `url(${require('../assets/images/' + slide)})`}"></div>
        </div>

        <div v-for="(section, index) in sections" :key="section.num_slides">
            <div v-for="slide in section.slides" :key="slide.id"
                 class="slide" :class="[slide.rectangle, slide.rectangle_mobile]" :style="styles(slide, index)" :ref="`slide_${section.num_slides}_${slide.id}`">
                <template v-if="!slide.form">
                    <div class="text" v-if="slide.text" v-html="slide.text"></div>
                    <Hover :url="slide.url" v-if="slide.link"/>
                    <Button class="button" :style="slide['button-style']" v-if="slide.button" :text="slide['button-text']" :url="slide['button-url']"/>
                </template>
                <ContactForm v-if="slide.form" :action="slide['form-action']"/>
            </div>
        </div>
...
export default {
    name: 'Home',
    components: { Loading, ContactForm, Button, Hover, NextIcon, MenuIcon, Menu },
    data () {
        return {
            sections,
            logo,
    ...
    methods: {
        next () {
            const section = this.sections[this.step]
            if (!section) {
                this.menu = true
                return
            }
            for (const slide of section.slides) {
                ...
                let from = slide.transition.from
                let to = slide.transition.to
                if (slide.rectangle_mobile === 'full-mobile' && window.screen.width < 768) {
                    to.left = '0%'
                    to.top = '0%'
                    from.left = '0%'
                    from.top = '-100%'
                ...
                fromTo(ref, Object.assign({}, from), Object.assign({}, to))
                this.animateText(section, slide)
            }
            this.runAdditionalTransitions(section, true)
            this.step++
        },

        prev () {
            const section = this.sections[this.step - 1]
            ...
            for (const slide of section.slides) {
                ...
                fromTo(ref, Object.assign({}, to), Object.assign({}, from)).then(() => {
                    this.hidePrevSteps()
                })
                this.animateText(section, slide)
            }

            this.runAdditionalTransitions(section, false)
            this.step--
        },
